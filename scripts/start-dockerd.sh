...
# Based on gVisor's start-dockerd.sh
# https://github.com/google/gvisor/blob/master/images/basic/docker/start-dockerd.sh
#
...
USE_OVERLAY_DRIVER=${USE_OVERLAY_DRIVER:-false}
USE_FUSE_OVERLAY_DRIVER=${USE_FUSE_OVERLAY_DRIVER:-true}
while [[ $# -gt 0 ]]; do
  case $1 in
    --overlay)
    ...
    --no-overlay)
      ...
      ;;
    --fuse-overlay)
      USE_FUSE_OVERLAY_DRIVER=true
      shift
      ;;
    --no-fuse-overlay)
      USE_FUSE_OVERLAY_DRIVER=false
      shift
      ;;
    *)
      shift
...
# Use fuse-overlayfs as the storage driver by default. This works better under
# gVisor than the regular overlay driver as it runs entirely in userspace.
if [[ "${USE_FUSE_OVERLAY_DRIVER}" == "true" ]]; then
  EXTRA_DOCKERD_FLAGS+=(--storage-driver=fuse-overlayfs)
fi
